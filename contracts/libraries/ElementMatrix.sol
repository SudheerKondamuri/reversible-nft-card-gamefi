...
library ElementMatrix {
    ...
    function getCombination(string memory e1, string memory e2) internal pure returns (CombinationResult memory) {
        ...
        if (equals(a, "Fire") && equals(b, "Fire")) return CombinationResult("Fire", 3, 0, "Double Flame: Deal double burn damage");
        if (equals(a, "Water") && equals(b, "Water")) return CombinationResult("Water", 0, 3, "Tidal Wave: Push all opponents back");
        if (equals(a, "Earth") && equals(b, "Earth")) return CombinationResult("Earth", 0, 4, "Diamond Skin: Reduce all incoming damage by 3");
        if (equals(a, "Air") && equals(b, "Air")) return CombinationResult("Air", 2, 2, "Tornado: Attack all opponents simultaneously");
        if (equals(a, "Lightning") && equals(b, "Lightning")) return CombinationResult("Lightning", 4, 2, "Overload: Chain lightning hits all enemies");

        // Cross-element combinations
        if (equals(a, "Fire") && equals(b, "Water")) return CombinationResult("Steam", 1, 1, "Obscure: Reduce opponent accuracy by 20%");
        ...
        return CombinationResult(a, 0, 0, "Mismatched: Unstable combination");
    }
}
